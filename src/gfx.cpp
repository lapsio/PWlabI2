...
void RenderEngine::flush(){
  ...
  for (int i = 0; i < this->objListLength ; i++){
    ...
    sp.setPosition(
          (this->objList[i].pos.getX()+this->objList[i].dec->offset.getX())*RenderEngine::UNITSIZE+pivot.getX(),
          (this->objList[i].pos.getY()+this->objList[i].dec->offset.getY())*RenderEngine::UNITSIZE+pivot.getY());
    this->window.draw(sp);
  }
  window.display();
}
